...
# Phase 2
## 수신부 프로젝트 구조 세팅
>송신과 수신 간의 완벽한 메모리 규격 동기화  
>
>규격 일치 시킴 (아두이노에서 사용했던 'packet.h'를 그대로 라즈베리파이 'receiver.cpp'의 헤더 파일로 사용함)  
>
>따라서 양측 시스템이 동일한 32B 패킷 구조체를 사용하도록 함  

## 리눅스 시리얼 포트 제어
>OS 간섭 원천 차단 및 고속 통신 환경 구축  
>
>C++ POSIX 시스템 헤더(`<termios.h>`, `<fcntl.h>`)를 활용하여 하드웨어 장치(`/dev/ttyACM0`)를 직접 개방함.  
>통신 속도는 921,600 로 고정함  
>
>Raw Mode 적용하여 리눅스 커널이 들어오는 바이너리 데이터를 오인하여 변형하지 못하도록 터미널 제어 간섭을 비활성화함  

## 프레임 동기화(Sync) 및 메모리 확보
>데이터 스트림 속에서 패킷의 시작점 탐색 (head 탐색)  
>
>블로킹(Blocking) 모드의 `read()` 함수를 통해 1바이트씩 스트림을 실시간 추적.  
>
>상태 머신(State Machine) 로직을 적용하여 사전에 약속된 Sync Head(`0xAA`)가 식별되는 순간부터 패킷 수집 상태로 전환하여 정확히 32바이트의 버퍼 배열을 확보함.  

## 구조체 Casting 및 데이터 해독
>수집된 바이너리 덩어리를 유효한 물리량 데이터로 번역  
>
>완성된 32바이트 버퍼 배열 위에 `reinterpret_cast`를 사용하여 `TelemetryPacket32B` 포인터를 직접 덮어씌우는 제로 카피(Zero-copy) 방식의 파싱 적용.  
>
>캐스팅 직후 데이터의 마지막 바이트가 Sync Tail(`0x55`)과 일치하는지 검증하여 프레임 엇갈림 및 밀림 현상을 원천 차단(드랍 처리).  
>
>정수 스케일링으로 전송된 센서 값(예: 온도 `2543`)을 배율에 맞춰 다시 실수형(`25.43°C`)으로 디코딩하여 검증을 완료함.  

## 중간 실행결과
![receiver.cpp 중간 실행결과](<images/receiver.cpp 중간 실행결과.png>)

>통신 연결 초기의 불안정한 데이터를 수신기가 스스로 걸러나고 정상궤도에 올린 상태임 
>
>패킷 유실률 0%  
>
